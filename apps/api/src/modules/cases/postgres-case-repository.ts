...
export class PostgresCaseRepository implements CaseRepository {
  ...
  async getCase(caseId: string): Promise<CaseRecord | null> {
    ...
    const exitModeResult = await this.pool.query<{
      exit_mode_ref: string;
      graph_version: number;
      account_ref: string;
      selected_mode: string;
      confidence: string | number;
      ranked_modes: unknown;
      features: unknown;
      feature_version: string;
      ...
      `SELECT
         ...
         em.model_version,
         em.rule_version,
         em.calculation_input_hash,
         em.signal_provenance,
         em.evaluated_at
       FROM exit_mode_snapshots em
       JOIN accounts a ON a.id = em.account_id
       WHERE em.case_id = $1
       ORDER BY em.graph_version`,
      [row.internal_case_id],
    );
    const exitModeSnapshots = exitModeResult.rows.map((exitRow) =>
      ExitModeSnapshotSchema.parse({
        exitModeRunId: exitRow.exit_mode_ref,
        caseId: row.external_case_id,
        accountId: exitRow.account_ref,
        graphVersion: exitRow.graph_version,
        selectedMode: exitRow.selected_mode,
        confidence: Number(exitRow.confidence),
        rankedModes: jsonValue(exitRow.ranked_modes),
        features: jsonValue(exitRow.features),
        featureVersion: exitRow.feature_version,
        modelVersion: exitRow.model_version,
        ruleVersion: exitRow.rule_version,
        calculationInputHash: exitRow.calculation_input_hash,
        signalProvenance: jsonValue(exitRow.signal_provenance),
        evaluatedAt: iso(exitRow.evaluated_at),
      }),
    );
    ...
    const gateResult = await this.pool.query<{
      ...
      geo_decision: unknown;
      time_decision: unknown;
      feature_version: string;
      rule_version: string;
      calculation_input_hash: string;
      evaluated_at: Date | string;
      ...
      `SELECT
         ...
         eg.feature_version,
         eg.rule_version,
         eg.calculation_input_hash,
         eg.evaluated_at
       FROM evidence_gate_snapshots eg
       JOIN accounts a ON a.id = eg.account_id
       WHERE eg.case_id = $1
       ORDER BY eg.graph_version`,
      [row.internal_case_id],
    );
    const evidenceGateSnapshots = gateResult.rows.map((gateRow) =>
      EvidenceGateSnapshotSchema.parse({
        evidenceGateRunId: gateRow.evidence_gate_ref,
        caseId: row.external_case_id,
        accountId: gateRow.account_ref,
        graphVersion: gateRow.graph_version,
        exitModeRunId: gateRow.exit_mode_ref,
        exitMode: gateRow.exit_mode,
        overallDecision: gateRow.overall_decision,
        geo: jsonValue(gateRow.geo_decision),
        time: jsonValue(gateRow.time_decision),
        featureVersion: gateRow.feature_version,
        ruleVersion: gateRow.rule_version,
        calculationInputHash: gateRow.calculation_input_hash,
  ...
  async saveCase(record: CaseRecord): Promise<void> {
    ...
    try {
      ...
      for (const snapshot of record.exitModeSnapshots) {
        const snapshotAccountId = await accountId(
          client,
          snapshot.signalProvenance.sourceName,
          snapshot.accountId,
        );
        const result = await client.query<{ exit_mode_ref: string }>(
          `INSERT INTO exit_mode_snapshots (
             exit_mode_ref, case_id, account_id, graph_version, selected_mode, confidence,
             ranked_modes, features, feature_version, model_version, rule_version,
             calculation_input_hash, signal_provenance, evaluated_at
           ) VALUES (
             $1, $2, $3, $4, $5, $6, $7::jsonb, $8::jsonb, $9, $10, $11, $12,
             $13::jsonb, $14
           )
           ON CONFLICT (case_id, graph_version) DO UPDATE SET
             evaluated_at = exit_mode_snapshots.evaluated_at
           WHERE exit_mode_snapshots.calculation_input_hash = EXCLUDED.calculation_input_hash
           RETURNING exit_mode_ref`,
          [
            snapshot.exitModeRunId,
            internalCaseId,
            snapshotAccountId,
            snapshot.graphVersion,
            snapshot.selectedMode,
            snapshot.confidence,
            JSON.stringify(snapshot.rankedModes),
            JSON.stringify(snapshot.features),
            snapshot.featureVersion,
            snapshot.modelVersion,
            snapshot.ruleVersion,
            snapshot.calculationInputHash,
            JSON.stringify(snapshot.signalProvenance),
        ...
        if (result.rows.length === 0) {
          throw new ConflictError(
            ...
            `Exit mode for graph version ${snapshot.graphVersion} already exists with different evidence`,
          );
        }
      }

      for (const snapshot of record.evidenceGateSnapshots) {
        const snapshotAccountId = await accountId(
          client,
          snapshot.geo.provenance.sourceName,
          snapshot.accountId,
        );
        const result = await client.query<{ evidence_gate_ref: string }>(
          `INSERT INTO evidence_gate_snapshots (
             evidence_gate_ref, case_id, account_id, graph_version, exit_mode_ref, exit_mode,
             overall_decision, geo_decision, time_decision, feature_version, rule_version,
             calculation_input_hash, evaluated_at
           ) VALUES (
             $1, $2, $3, $4, $5, $6, $7, $8::jsonb, $9::jsonb, $10, $11, $12, $13
           )
           ON CONFLICT (case_id, graph_version) DO UPDATE SET
             evaluated_at = evidence_gate_snapshots.evaluated_at
           WHERE evidence_gate_snapshots.calculation_input_hash = EXCLUDED.calculation_input_hash
           RETURNING evidence_gate_ref`,
          [
