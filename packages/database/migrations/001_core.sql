...
CREATE TABLE exit_mode_snapshots (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  exit_mode_ref text NOT NULL UNIQUE,
  case_id uuid NOT NULL REFERENCES cases(id),
  account_id uuid NOT NULL REFERENCES accounts(id),
  graph_version integer NOT NULL CHECK (graph_version > 0),
  selected_mode text NOT NULL CHECK (selected_mode IN ('STATIONARY', 'FORWARD', 'CASH_OUT_LIKELY')),
  confidence numeric(5,4) NOT NULL CHECK (confidence >= 0 AND confidence <= 1),
  ranked_modes jsonb NOT NULL,
  features jsonb NOT NULL,
  feature_version text NOT NULL,
...
CREATE TABLE evidence_gate_snapshots (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  evidence_gate_ref text NOT NULL UNIQUE,
  case_id uuid NOT NULL REFERENCES cases(id),
  account_id uuid NOT NULL REFERENCES accounts(id),
  graph_version integer NOT NULL CHECK (graph_version > 0),
  exit_mode_ref text NOT NULL REFERENCES exit_mode_snapshots(exit_mode_ref),
  exit_mode text NOT NULL CHECK (exit_mode IN ('STATIONARY', 'FORWARD', 'CASH_OUT_LIKELY')),
  overall_decision text NOT NULL CHECK (overall_decision IN ('PREDICT', 'PARTIAL', 'ABSTAIN')),
  geo_decision jsonb NOT NULL,
  time_decision jsonb NOT NULL,
  feature_version text NOT NULL,
  rule_version text NOT NULL,
  calculation_input_hash char(64) NOT NULL,
  evaluated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE INDEX exit_mode_case_time_idx ON exit_mode_snapshots (case_id, evaluated_at DESC);
CREATE INDEX evidence_gate_case_time_idx ON evidence_gate_snapshots (case_id, evaluated_at DESC);
